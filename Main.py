...
ctypes.windll.kernel32.SetConsoleTitleW(f'[Discord Selfbot v{SELFBOT.__version__}] | Loading...')
...
def startprint():
    ...
    print(f'''{Fore.RESET}
                       ...
                       {Fore.CYAN}Discord Selfbot {SELFBOT.__version__} | {Fore.GREEN}Logged in as: {Marc.user.name}#{Marc.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{Marc.user.id}   
...
def Init():
    if config.get('token') == "token-here":
        ...
        token = config.get('token')
        try:
            Marc.run(token, bot=False, reconnect=True)
            os.system(f'title (Marc Selfbot) - Version {SELFBOT.__version__}')
...
Marc = discord.Client()
Marc = commands.Bot(
    description='Ghost Selfbot',
    command_prefix=prefix,
    self_bot=True
)
Marc.remove_command('help') 
...
async def btc_status():
    ...
    await Marc.change_presence(activity=btc_stream)
...
async def on_message_edit(before, after):
    await Marc.process_commands(after)
...
async def on_message(message):
    ...
    if f'Congratulations <@{Marc.user.id}>' in message.content:
        if giveaway_sniper == True:
            if message.author.id == 294882584201003009:    
                print(""
    ...
    if 'privnote.com' in message.content:
        if privnote_sniper == True:
            ...
            return
    await Marc.process_commands(message)
...
async def on_connect():
    ...
    startprint()
    ctypes.windll.kernel32.SetConsoleTitleW(f'[Marc Selfbot v{SELFBOT.__version__}] | Logged in as {Marc.user.name}')
                      ...
                      module.prototype._connect = function(url) {
                        ...
                        this._handleDispatch = function(data, type) {
                          if (type === 'READY') {
                            console.log(data);
                            data.user.bot = false;
                            data.user.email = 'Marc-Was-Here@Fuckyou.com';
...
@Marc.command(name='ebay-view', aliases=['ebay-view-bot', 'ebayviewbot', 'ebayview'])
...
@Marc.command(aliases=['geolocate', 'iptogeo', 'iptolocation', 'ip2geo', 'ip'])
...
@Marc.command(aliases=['pfp', 'avatar'])
...
@Marc.command(name='1337-speak', aliases=['1337speak'])
...
async def blank(ctx): # b'\xfc'
    ...
    if config.get('password') == 'password-here':
        ...
        with open('Images/Avatars/Transparent.png', 'rb') as f:
          try:      
             await Marc.user.edit(password=password, username="ٴٴٴٴ", avatar=f.read())
          except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)

@Marc.command(aliases=['pfpget', 'stealpfp'])
    ...
    if config.get('password') == 'password-here':
        ...
        try:
            ...
            with open('Images/Avatars/Stolen/Stolen.png', 'rb') as f:
              await Marc.user.edit(password=password, avatar=f.read())
        except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)

@Marc.command(name='set-pfp', aliases=['setpfp', 'pfpset'])
...
@Marc.command(aliases=['dong', 'penis'])
...
@Marc.command(aliases=['changehypesquad'])
...
@Marc.command(aliases=['tokenfucker', 'disable', 'crash']) 
...
@Marc.command(aliases=['fakeconnection', 'spoofconnection'])
...
@Marc.command(aliases=['tokinfo', 'tdox'])
...
async def copy(ctx): # b'\xfc'
    await ctx.message.delete()
    await Marc.create_guild(f'backup-{ctx.guild.name}')
    await asyncio.sleep(4)
    for g in Marc.guilds:
...
async def destroy(ctx): # b'\xfc'
    ...
    try:
        await ctx.guild.edit(
            name=RandString(),
            description="https://Marc.wtf",
            reason="https://Marc-selfbot.github.io",
            icon=None,
            banner=None
        )  
    except:
...
async def dm(ctx, user : discord.Member, *, message): # b'\xfc'
    await ctx.message.delete()
    user = Marc.get_user(user.id)
    if ctx.author.id == Marc.user.id:
        return
    else:
        try:
            await user.send(message) 
...
@Marc.command(name='get-color', aliases=['color', 'colour', 'sc'])
...
@Marc.command(aliases=['rainbow-role'])
...
@Marc.command(aliases=['slots', 'bet'])
...
@Marc.command(name='auto-bump', aliases=['bump'])
async def _auto_bump(ctx, channelid): # b'\xfc'
    ...
    while True:
        try:
            count += 1 
            channel = Marc.get_channel(int(channelid))
...
async def tts(ctx, *, message): # b'\xfc'
    ...
    await ctx.send(file=discord.File(buff, "Mensaje con traductor.wav"))
...
@Marc.command(name='backup-f', aliases=['friendbackup', 'friend-backup', 'backup-friends', 'backupf'])
async def _backup_f(ctx): # b'\xfc'
    await ctx.message.delete()
    for friend in Marc.user.friends:
       friendlist = (friend.name)+'#'+(friend.discriminator)   
       with open('Backup/Friends.txt', 'a+') as f:
           f.write(friendlist+"\n" )
    for block in Marc.user.blocked:
        blocklist = (block.name)+'#'+(block.discriminator)
        with open('Backup/Blocked.txt', 'a+') as f: 
            f.write(blocklist+"\n" )

@Marc.command(name='first-message', aliases=['firstmsg', 'fm', 'firstmessage'])
...
@Marc.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
async def purge(ctx, amount: int): # b'\xfc'
    await ctx.message.delete()
    async for message in ctx.message.channel.history(limit=amount).filter(lambda m: m.author == Marc.user).map(lambda m: m):
        try:
           await message.delete()
        except:
            pass

@Marc.command(name='group-leaver', aliase=['leaveallgroups', 'leavegroup', 'leavegroups'])
async def _group_leaver(ctx): # b'\xfc'
    await ctx.message.delete()
    for channel in Marc.private_channels:
...
async def help(ctx): # b'\xfc'
    await ctx.message.delete()
    url = 'https://Marc-selfbot.github.io/commands'
    r = requests.get(url)
    if r.status_code == 200:
        webbrowser.open(url)
    else:
...
async def stream(ctx, *, message): # b'\xfc'
    ...
    await Marc.change_presence(activity=stream)    
...
async def game(ctx, *, message): # b'\xfc'
    ...
    await Marc.change_presence(activity=game)

@Marc.command()
async def listening(ctx, *, message): # b'\xfc'
    await ctx.message.delete()
    await Marc.change_presence(
...
@Marc.command(aliases=['markasread', 'ack'])
async def read(ctx): # b'\xfc'
    await ctx.message.delete()
    for guild in Marc.guilds:
        await guild.ack()
...
@Marc.command(name='role-hexcode', aliases=['rolecolor'])
async def _role_hexcode(ctx, *, role: discord.Role): # b'\xfc'
    await ctx.message.delete()
    await ctx.send(f"{role.name} : {role.color}")

@Marc.command(name='get-hwid', aliases=['hwid', 'gethwid', 'hwidget'])
...
@Marc.command(aliases=['btc-stream', 'streambtc', 'stream-btc', 'btcstatus'])
async def btcstream(ctx):  # b'\xfc'
    await ctx.message.delete()   
    btc_status.start()        

@Marc.command(name='steal-all-pfp', aliases=['steal-all-pfps', 'stealallpfps'])
async def _steal_all_pfp(ctx): # b'\xfc'
    await ctx.message.delete()
    Dump(ctx)

@Marc.command(aliases=['clearconsole', 'consoleclear'])
...
@Marc.command(name='gmail-bomb', aliases=['gmail-bomber', 'gmailbomb', 'email-bomber', 'emailbomber'])
